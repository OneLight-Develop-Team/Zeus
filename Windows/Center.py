...
class CenterWindow(QWidget):
    ...
    def addSource(self, tag):
        ...
        # for type in alldoccol.find({"Type":1}):    #读取文件类型库里文件类型名
        #     assetdb = client[type]  #获得文件类型名对应的资产数据库
        #     collist = assetdb.list_collection_names()   #资产数据库里集合名列表
        #     for file in collist:    #取出资产数据库里单个列表名，即为文件名
        #         assetPath = assetdb[file].find({"_id":"Path"}, {"Path":1})
        #         path = assetPath["Path"]
        #         assetType = assetdb[file].find({"_id": "FileType"}, {"FileType": 1})
        #         type = assetType["FileType"]
        #         assetName = assetdb[file].find({"_id": "FileName"}, {"FileName": 1})
        #         name = assetName["FileName"]
        #         print(path + "***" + type + "***" + name+ "***")
        #         self.addFile(path, type, name)
